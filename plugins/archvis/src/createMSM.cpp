...
megamol::archvis::CreateMSM::CreateMSM()
    ...
    , m_version(0) {
    this->m_getData_slot.SetCallback(CallMSMData::ClassName(), "GetData", &CreateMSM::getDataCallback);
    this->MakeSlotAvailable(&this->m_getData_slot);

    // TODO GetExtents?

    this->m_node_floatTable_slot.SetCompatibleCall<megamol::stdplugin::datatools::table::TableDataCallDescription>();
    this->MakeSlotAvailable(&this->m_node_floatTable_slot);

    this->m_element_floatTable_slot.SetCompatibleCall<megamol::stdplugin::datatools::table::TableDataCallDescription>();
...
bool megamol::archvis::CreateMSM::getDataCallback(core::Call& caller) {
    ...
    CallMSMData* msm_call = dynamic_cast<CallMSMData*>(&caller);
    if (msm_call == NULL) {
        return false;
    }
        ...
        if (element_ft->GetColumnsCount() == 4) {
            ...
            for (int elem_idx = 0; elem_idx < element_ft->GetRowsCount(); ++elem_idx) {
                ...
                elements.push_back({type, static_cast<int>(elem_accesssor[curr_idx + 0]) - 1,
                    static_cast<int>(elem_accesssor[curr_idx + 1]) - 1,
                    static_cast<int>(elem_accesssor[curr_idx + 2]) - 1,
